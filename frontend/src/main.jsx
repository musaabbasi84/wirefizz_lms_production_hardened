...
/* Tokens travel in the Authorization header (no cookies), so the SPA on Vercel and the API on Render can be on different domains. */
const TOKEN_KEYS={access:'wf_access_token',refresh:'wf_refresh_token'};
const memoryTokens={};
const store={
  get(kind){try{const v=window.localStorage.getItem(TOKEN_KEYS[kind]);return v||memoryTokens[kind]||null}catch{return memoryTokens[kind]||null}},
  set(kind,value){memoryTokens[kind]=value;try{window.localStorage.setItem(TOKEN_KEYS[kind],value)}catch{}},
  clear(){delete memoryTokens.access;delete memoryTokens.refresh;try{window.localStorage.removeItem(TOKEN_KEYS.access);window.localStorage.removeItem(TOKEN_KEYS.refresh)}catch{}}
};
const saveSession=d=>{if(d?.access_token)store.set('access',d.access_token);if(d?.refresh_token)store.set('refresh',d.refresh_token)};
const NETWORK_MSG='Cannot reach the server. It may be waking up - please wait a few seconds and try again.';
const parseBody=async r=>{try{return await r.json()}catch{return null}};
let refreshInFlight=null;
const performRefresh=usedAccess=>{
  if(refreshInFlight)return refreshInFlight;
  const run=async()=>{
    /* another tab may already have rotated the tokens while we waited for the lock */
    const current=store.get('access');
    if(usedAccess&&current&&current!==usedAccess)return;
    const rt=store.get('refresh');
    if(!rt)throw new ApiError('No refresh session',401);
    let r;try{r=await fetch(`${API}/api/auth/refresh`,{method:'POST',headers:{Authorization:`Bearer ${rt}`}})}catch{throw new ApiError(NETWORK_MSG,0)}
    const d=await parseBody(r);
    if(!r.ok)throw new ApiError(d?.error||`Refresh failed (${r.status})`,r.status);
    saveSession(d);
  };
  const locks=typeof navigator!=='undefined'?navigator.locks:null;
  refreshInFlight=(locks?locks.request('wf-refresh',run):run()).finally(()=>{refreshInFlight=null});
  return refreshInFlight;
};
const expireSession=message=>{store.clear();window.dispatchEvent(new CustomEvent('wf:auth-expired',{detail:message||'Your session has expired. Please sign in again.'}))};
const request=async(path,opts={},retry=true)=>{
  const {_blob,...fetchOpts}=opts;
  const token=store.get('access');
  const hasBody=fetchOpts.body!==undefined&&fetchOpts.body!==null;
  const headers={...(hasBody&&!(fetchOpts.body instanceof FormData)?{'Content-Type':'application/json'}:{}),...(token?{Authorization:`Bearer ${token}`}:{}),...(fetchOpts.headers||{})};
  let r;try{r=await fetch(`${API}${path}`,{...fetchOpts,headers})}catch{throw new ApiError(NETWORK_MSG,0)}
  const isAuthForm=path==='/api/auth/login'||path==='/api/auth/register';
  if(r.status===401&&retry&&!isAuthForm&&store.get('refresh')){
    try{await performRefresh(token)}catch(e){
      if(e.status===401||e.status===403){expireSession();throw new ApiError('Your session has expired. Please sign in again.',401)}
      throw e;
    }
    return request(path,opts,false);
  }
  if(!r.ok){const d=await parseBody(r);throw new ApiError(d?.error||`Request failed (${r.status})`,r.status)}
  if(_blob)return r.blob();
  return parseBody(r);
};
...
const downloadFile=async(path,filename)=>{const blob=await request(path,{_blob:true});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=filename;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)};
...
function App(){const[user,setUser]=useState(null);const[page,setPage]=useState('dashboard');const[boot,setBoot]=useState(true);const[bootError,setBootError]=useState('');const[notice,setNotice]=useState('');const[attempt,setAttempt]=useState(0);
useEffect(()=>{let cancelled=false;setBoot(true);setBootError('');
(async()=>{try{
  if(!store.get('access')&&!store.get('refresh')){if(!cancelled)setUser(null);return}
  const d=await get('/api/auth/me');
  if(!cancelled)setUser(d.user);
}catch(err){
  if(cancelled)return;
  if(err.status===0){setBootError(err.message);return}
  if(err.status===401||err.status===403)store.clear();
  setUser(null);
}finally{if(!cancelled)setBoot(false)}})();
return()=>{cancelled=true}},[attempt]);
useEffect(()=>{const expired=e=>{setUser(null);setPage('dashboard');setNotice(e?.detail||'Your session has expired. Please sign in again.')};window.addEventListener('wf:auth-expired',expired);return()=>window.removeEventListener('wf:auth-expired',expired)},[]);
if(boot)return <div className="loading">Loading WireFizz LMS…</div>;
if(bootError)return <div className="error-screen"><div className="panel"><span className="eyebrow">CONNECTION</span><h2>Server unavailable</h2><p className="muted">{bootError}</p><button className="primary" onClick={()=>setAttempt(a=>a+1)}>Try again</button></div></div>;
if(!user)return <Login notice={notice} onLogin={u=>{setNotice('');setUser(u);setPage('dashboard')}}/>;
const logout=async()=>{const rt=store.get('refresh');store.clear();setNotice('');setUser(null);setPage('dashboard');try{await fetch(`${API}/api/auth/logout`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({refresh_token:rt})})}catch{}};
return <Shell user={user} onLogout={logout} setPage={setPage}><ErrorBoundary key={page}><PageView user={user} page={page} setPage={setPage} setUser={setUser}/></ErrorBoundary></Shell>}
